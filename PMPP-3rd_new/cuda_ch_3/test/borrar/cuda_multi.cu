...
void sum_matrix(int** dd_mat_a,int** dd_mat_b,int** dd_mat_c, int n, int m){
	...
	if( y<n && x<m ){ // revisar <<<<<<<<<<<<<<<<<<<<<<<<<<<<
		//*(*(dd_mat_a+y)+x)=-9;
		*(*(dd_mat_c+y)+x) += *(*(dd_mat_a+y)+x) + *(*(dd_mat_b+x)+y);
	}
}
*/

__global__ 
void sum_multi(int** dd_mat_a,int** dd_mat_b,int** dd_mat_c, int n, int m){
	int i=0;
	int j=0;
	int k=0;
	...
	for(i=0;i<n;i++){
		for(j=0;j<m;j++){
			for(k=0;k<n;k++){
				temp += *(*(dd_mat_a+i)+k) * *(*(dd_mat_b+k)+j);
			}
			*(*(dd_mat_c+i)+j)=temp ;
			temp=0;
		}
...
__global__ 
void sum_multi_shared(int** dd_mat_a,int** dd_mat_b,int** dd_mat_c, int n, int m){
	__shared__ int temp[m];
	
	//int x = threadIdx.x + blockIdx.x*blockDim.x;
	//int y = threadIdx.y + blockIdx.y*blockDim.y;

	int fil =blockIdx.y*blockDim.y;
	int col =blockIdx.x*blockDim.x;

	if( threadIdx.x<m ){
		temp[threadIdx.x]= *(*(dd_mat_a+y)+x+threadIdx.x) + *(*(dd_mat_b+x+threadIdx.x)+y);	
...
*/
	...
	sum_multi<<<grid,blockNum>>>(dd_mat_a,dd_mat_b,dd_mat_c,n,m);
