...
void matrix_mult_shared(int** dd_mat_a,int** dd_mat_b,int** dd_mat_c, int width){
	...
	for(k=0 ; k<width/WIDTH_TILE ; k++){
		//Mds[ty][tx]=*(*(dd_mat_a)+fil*width+k*WIDTH_TILE+tx);
		Mds[ty][tx]=dd_mat_a[fil][k+WIDTH_TILE+tx];
		//Nds[ty][tx]=*(*(dd_mat_b)+(k*WIDTH_TILE+ty)*width+col);
...
int main(){
	...
	dim3 blockNum(WIDTH_TILE,WIDTH_TILE);
	dim3 grid((n + WIDTH_TILE-1)/blockNum.x,(m + WIDTH_TILE-1)/blockNum.y);
